...
for i in range(0,len(listsOfQuestions)):
    ...
    if ans ==listsOfQuestions[i][-1]:
        print("Correct")
        treward = reward[i]  
        print("Your points",treward,"/",(i+1)*10) 
    else:
        cut = cut + 1
        treward = reward[i]
        treward2 = treward-(cut*10)
        print("Wrong!---------> correct:",end=""),print(listsOfQuestions[i][listsOfQuestions[i][-1]]),print("Your points",treward2,"/",(i+1)*10)
    print("")

print("you got",treward,"/",100)